...
def merge_main_csv():
    ...
    file_paths = glob.glob('data/hand_angles*.csv')  # 모든 CSV 파일을 가져옴
...
def merge_test_csv():
    # CSV 파일들의 경로 패턴 설정 (현재 디렉토리에 있는 csv 파일을 모두 읽어온다고 가정)
    file_paths = glob.glob('data/test_data*.csv')  # 모든 CSV 파일을 가져옴
    ...
    # 합친 데이터프레임을 새로운 CSV 파일로 저장
    merged_df.to_csv('data/merged_test_data.csv', index=False)
